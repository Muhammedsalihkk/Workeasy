...
export const owner_autherization=(req:Request,res:Response,next:NextFunction)=>{
...
export const issuper_admin=(req:Request,res:Response,next:NextFunction)=>{
    const {token}=req.cookies
    if(!token)
    {
         res.status(401).json({message:"unauthrized"})
